...
const config = require('../config/giftcardFilters.json');

test.describe('GET /v1/payout/giftcard/filters', () => {
  ...
  test('VALID country (from config)', async ({ request }) => {
    ...
    if (!country) {
      throw new Error('Country is missing in config/giftcardFilters.json');
    }

    console.log('Using country:', country);

    const response = await request.get(
      `${BASE_URL}/v1/payout/giftcard/filters?country=${country}`,
      {
        headers: {
          accept: '*/*',
