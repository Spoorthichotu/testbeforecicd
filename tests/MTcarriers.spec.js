...
const config = require('../config/MTcarriers.json');

test.describe('GET /v1/topup/carriers', () => {
  ...
  test('VALID countryCode (from config)', async ({ request }) => {
    ...
    if (!countryCode) {
      throw new Error('countryCode is missing in config/MTcarriers.json');
    }

    console.log('Using countryCode:', countryCode);

    const response = await request.get(
      `${BASE_URL}/v1/topup/carriers?countryCode=${countryCode}`,
      {
        headers: {
          accept: '*/*',
